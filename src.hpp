...
class complex {
  ...
  public:
    ...
    friend std::ostream &operator << (std::ostream &os, const complex &x) {
        ...
        if (sign(real_part) == 0) real_part = 0.0;  // 归零
        os << std::fixed << std::setprecision(6) << real_part;

        // 虚部处理
        double imag_part = x.b;
        if (sign(imag_part) == 0) imag_part = 0.0;  // 归零

        // 虚部为负数时输出负号，否则输出加号
        if (sign(imag_part) < 0) {
            os << std::fixed << std::setprecision(6) << imag_part << "i";
        } else {
            os << "+" << std::fixed << std::setprecision(6) << imag_part << "i";
        }
        return os;
